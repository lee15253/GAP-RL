...
def convert_observation_to_space(observation, prefix=""):
    ...
    elif isinstance(observation, (torch.FloatTensor, torch.cuda.FloatTensor)):
        shape = observation.shape
        logger.debug(f"The observation ({prefix}) is a (float tensor) scalar")
        space = spaces.Box(-torch.inf, torch.inf, shape=shape, dtype=np.float32)
    ...
    elif isinstance(observation, (torch.IntTensor, torch.cuda.IntTensor)):
        logger.debug(f"The observation ({prefix}) is a (integer tensor) scalar")
        space = spaces.Box(-torch.inf, torch.inf, shape=[1], dtype=int)
    ...
    elif isinstance(observation, (torch.BoolTensor, torch.cuda.BoolTensor)):
        logger.debug(f"The observation ({prefix}) is a (bool) scalar")
        space = spaces.Box(0, 1, shape=[1], dtype=np.bool_)
    else:
