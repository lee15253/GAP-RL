...
import numpy as np
import tqdm
from .cv2_utils import visualize_depth
...
def put_info_on_image(image, info: Dict[str, Union[int, float, np.ndarray]], extras=None, overlay=True):
    ...
    for k, v in info.items():
        if isinstance(v, int):
            ...
            lines.append(f"{k}: {np.array2string(v, precision=3, suppress_small=True, formatter={'float': '{:.3f}'.format})}")
        elif isinstance(v, dict):
            # for vk, vv in v.items():
            #     lines.append(f"{vk}: {vv}")
            key_vals = ",".join([f"{k}" for k in v.keys()]) + f": " + ",".join([f"{val:.3f}" for val in v.values()])
            lines.append(key_vals)
        else:
