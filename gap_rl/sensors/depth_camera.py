...
class StereoDepthCameraConfig(CameraConfig):
    def __init__(self, *args, min_depth: float = 0.05, ir_height: int = 720, ir_width: int = 1280, **kwargs):
    ...
    @property
    def ir_resolution(self):
        return (self.ir_width, self.ir_height)

    @property
    def ir_intrinsic(self):
        fy = (self.ir_height / 2) / np.tan(self.fov / 2)
        return np.array([[fy, 0, self.ir_width / 2], [0, fy, self.ir_height / 2], [0, 0, 1]])
...
class StereoDepthCamera(Camera):
    def __init__(
        ...
        sensor_config.ir_resolution = camera_cfg.ir_resolution
        sensor_config.ir_intrinsic = camera_cfg.ir_intrinsic
