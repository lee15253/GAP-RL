...
class Camera:
    ...
    def get_texture(self, name):
        ...
        if dtype == "float":
            return self.camera.get_float_texture(name)
        elif dtype == "uint32":
            return self.camera.get_uint32_texture(name)
        else:
            raise NotImplementedError(f"Unsupported texture type: {dtype}")

    def get_camera_rgb(self, uint8=True):
        image = self.get_texture("Color")[..., :3]  # [H, W, 3]
        if uint8:
    ...
    def get_camera_depth(self):
        ...
        # The unit is meter
        depth = -self.get_texture("Position")[..., [2]]  # [H, W, 1]
        # mask = get_texture(camera, "Position")[..., [3]]
        # depth[mask == 1] = camera.far
        return depth

    def get_camera_seg(self):
        seg = self.get_texture("Segmentation")  # [H, W, 4]
    ...
    def get_camera_pcd(
        ...
        if rgb:
            pcd["rgb"] = self.get_camera_rgb()[mask]
        if visual_seg or actor_seg:
            seg = self.get_camera_seg()
            if visual_seg:
