...
def transform_pred_grasp_pcs(grasp_pred, device="cuda"):
    ...
    bs_gripper_points = th.unsqueeze(adjust_gripper_points, 0).repeat(grasp_shape[0], 1, 1)  # (bs, 6, 3)

    num_points = bs_gripper_points.shape[1]
    input_pred_grasps = grasp_pred  # (bs, 7)
    pred_grasps = th.unsqueeze(input_pred_grasps, 1).repeat(1, num_points, 1)  # (bs, 6, 7)
...
def goal_pred_posquat_loss(grasp_pred, grasp_goal):
    ...
    rot_dist1 = 1 - th.clamp(th.abs(th.sum(grasp_pred[:, 4:] * grasp_goal[:, 4:], dim=-1)), min=0, max=1)
...
class CustomSAC(SAC):
    ...
    def __init__(
        ...
        if distill:
            rl_model_path = "20240403_163000/rl_model_2000000_steps"
            self.rl_teacher_model = SAC.load(rl_model_path)
            self.rl_teacher_model.actor.features_extractor.eval()
            for param in self.rl_teacher_model.actor.features_extractor.parameters():
                param.require_grad = False
            self.rl_teacher_model.critic.features_extractor.eval()
            for param in self.rl_teacher_model.critic.features_extractor.parameters():
                param.require_grad = False
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        actor_distill_losses, critic_distill_losses = [], []

        for gradient_step in range(gradient_steps):
            # Sample replay buffer
            replay_data = self.replay_buffer.sample(batch_size, env=self._vec_normalize_env)
            close_grasp_pose_ee = replay_data.observations["close_grasp_pose_ee"]  # (bs, n, 6, 3)
            ...
            if self.distill:
                with th.no_grad():
                    teacher_actor_pn_feat = self.rl_teacher_model.actor.features_extractor(replay_data.observations)[..., 128:256]
                    teacher_critic_pn_feat = self.rl_teacher_model.critic.features_extractor(replay_data.observations)[..., 128:256]
                # actor_pn_feat = self.actor.features_extractor.gpg(replay_data.observations["gripper_pts_diff"])
                # critic_pn_feat = self.critic.features_extractor.gpg(replay_data.observations["gripper_pts_diff"])
                actor_pn_feat = self.actor.features_extractor(replay_data.observations)[..., 128:256]
                critic_pn_feat = self.critic.features_extractor(replay_data.observations)[..., 128:256]

                # weight = 10 * 0.98 ** (self.num_timesteps // 20000)
                weight = 10
                actor_distill_loss = F.mse_loss(teacher_actor_pn_feat, actor_pn_feat) * weight
                actor_distill_losses.append(actor_distill_loss.item())
                critic_distill_loss = F.mse_loss(teacher_critic_pn_feat, critic_pn_feat) * weight
                critic_distill_losses.append(critic_distill_loss.item())

            # Compute critic loss
            critic_loss = (
                0.5 * sum(F.mse_loss(current_q, target_q_values) for current_q in current_q_values) + critic_aux_loss
            )
            if self.distill:
                critic_loss += critic_distill_loss
        ...
        if self.distill:
            self.logger.record("train/actor_distill_loss", np.mean(actor_distill_losses))
            self.logger.record("train/critic_distill_loss", np.mean(critic_distill_losses))
