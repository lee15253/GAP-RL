...
class Robotiq2FingerGripper:
    ...
    def getStatus(self):
        """Request the status from the gripper and return it in the
        Robotiq2FGripper_robot_input msg type."""
    ...
    def goto(self, pos, vel, force):
        ...
        self.rSP = int(np.clip(255. / (self._max_velo) * vel, 0, 255))
        self.rFR = int(np.clip(255. / (self._max_force) * force, 0, 255))
        # print(self.rPR, self.rSP, self.rFR)
        self._update_cmd()
...
if __name__ == '__main__':
    ...
    print(gripper.getStatus(), gripper.is_ready())
    gripper.activate_gripper()
    gripper.sendCommand()
    print(gripper.getStatus(), gripper.is_ready())
    gripper.goto(pos=0, vel=0.05, force=10)
    gripper.sendCommand()
