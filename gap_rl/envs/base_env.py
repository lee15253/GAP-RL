...
from gap_rl.sensors.depth_camera import (
    StereoDepthCamera,
...
class BaseEnv(gym.Env):
    """Superclass for ChallengeGrasp environments.
    Args:
    ...
    def get_reward(self, **kwargs):
        if self._reward_mode == "sparse":
            ...
            return float(eval_info["is_success"])
    ...
    def _load_background(self):
        ...
        if self.bg_name != "minimal_bedroom":
            raise NotImplementedError(f"Unsupported background: {self.bg_name}")
        ...
        if not path.exists():
            raise FileNotFoundError(
                ...
                "Please download the background asset by `python -m gap_rl.utils.download_asset {}`".format(
                    self.bg_name
                )
            )
    ...
    def reset(self, seed=None, reconfigure=False):
        ...
        action = np.zeros(self.agent.action_space.shape[0])
        return self.get_obs(action)

    def set_episode_rng(self, seed):
        """Set the random generator for current episode."""
        self._episode_seed = self._main_rng.randint(2 ** 32) if seed is None else seed
    ...
    def step_action(self, action):
        ...
        for sim_step in range(self._sim_steps_per_control):
            ...
            self._scene.step()
            self._after_simulation_step(sim_step)
    ...
    def get_done(self, info: dict, **kwargs):
        ...
        # NOTE(pwxie): to avoid episode early terminate
        return False

    def get_info(self, **kwargs):  # sourcery skip: dict-assign-update-to-union
    ...
    def get_sim_state(self) -> np.ndarray:
        ...
        state = [get_actor_state(actor) for actor in self._actors]
        state.extend(
            get_articulation_state(articulation) for articulation in self._articulations
        )
    ...
    def render(self, mode="human", **kwargs):
        ...
        if mode == "human":
            if self._viewer is None:
                self._viewer = Viewer(self._renderer, resolutions=(1600, 900))
                self._setup_viewer()
            self._viewer.render()
            return self._viewer
        elif mode == "rgb_array":
            ...
            for camera in self._render_cameras.values():
                ...
                images.append(rgb)
            return images[0] if len(images) == 1 else tile_images(images)
        elif mode == "render_cameras":
            return self._extracted_from_render_camera()
        elif mode == "agent_cameras":
            return self._extracted_from_agent_cameras()
        elif mode == "cameras":
            return self._extracted_from_render_()
        else:
            raise NotImplementedError(f"Unsupported render mode {mode}.")

    def _extracted_from_render_(self):
        images = [self.render("rgb_array")] if len(self._render_cameras) > 0 else []
    ...
    def _extracted_from_render_camera(self):
        assert "base_camera" in self._cameras.keys() and "hand_camera" in self._cameras.keys()
        images = []
        if len(self._render_cameras) > 0:
            for camera in self._render_cameras.values():
                ...
                rgb = np.clip(rgba[..., :3] * 255, 0, 255).astype(np.uint8)
                images.append(rgb)
            images = [images[0] if len(images) == 1 else tile_images(images)]
        self.update_render()
        self.take_picture()
        for name in ["base_camera", "hand_camera"]:
            cam = self._cameras[name]
            images.extend(observations_to_images(cam.get_images()))
        return tile_images(images)

    def _extracted_from_agent_cameras(self):
        assert "hand_realsense" in self._cameras.keys()
        images = []
        if len(self._render_cameras) > 0:
            for camera in self._render_cameras.values():
                rgba = camera.get_images(take_picture=True)["Color"]
        ...
        self.update_render()
        self.take_picture()
        for name in ["hand_realsense"]:  # "base_kinect"
            cam = self._cameras[name]
            images.extend(observations_to_images(cam.get_images()))
        return tile_images(images)
    ...
    def gen_scene_pcd(self, num_points: int = int(1e5)) -> np.ndarray:
        ...
        for articulation in articulations:
            if articulation_mesh := merge_meshes(get_articulation_meshes(articulation)):
                meshes.append(articulation_mesh)

        for actor in self._scene.get_all_actors():
            if actor_mesh := merge_meshes(get_actor_meshes(actor)):
                meshes.append(
        ...
        return scene_mesh.sample(num_points)

    def get_fused_pointcloud(
        self, cams=None, rgb=False, visual_seg=False, actor_seg=False
    ) -> Dict[str, np.ndarray]:
        """get pointcloud from each camera, transform them to the *world* frame, and fuse together"""
        if cams is None:  # "hand_realsense", "base_kinect"
            cams = ["hand_realsense"]
        ...
        for cam_key in cams:
            ...
            cam.take_picture()
            pcd = cam.get_camera_pcd(rgb=rgb, visual_seg=visual_seg, actor_seg=actor_seg)  # dict
            # Model matrix is the transformation from OpenGL camera space to SAPIEN world space
            # camera.get_model_matrix() must be called after scene.update_render()!
            trans_cam2world = cam.camera.get_model_matrix()
            pcd["xyz"] = transform_points(trans_cam2world, pcd["xyz"])
            pcds.append(pcd)
        fused_pcd = {}
    ...
    # def get_fused_stereopc(
    #     self, cams=None, rgb=True, visual_seg=False, actor_seg=False
    # ) -> Dict[str, np.ndarray]:
    #     """get pointcloud from each camera, transform them to the *world* frame, and fuse together"""
    #     if cams is None:  # "hand_realsense", "base_kinect"
    #         cams = ["hand_realsense"]
    ...
    #         cam = self._cameras[cam_key]
    #         cam.take_picture()
    #         pcd = cam.get_camera_pcd(rgb, visual_seg, actor_seg)  # dict
    #         # Model matrix is the transformation from OpenGL camera space to SAPIEN world space
    #         # camera.get_model_matrix() must be called after scene.update_render()!
    #         T = cam.get_params()["cam2world_gl"]
    #         R = T[:3, :3]
    #         t = T[:3, 3]
